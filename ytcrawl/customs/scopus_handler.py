...
class ScopusHandler:
    ...
    def plot_box_by_video(self, where=None, days_from=None, days_until=None, scopus_column="Cited by"):
        ...
        _wo_videos_cit = np.log10(self.df_scopus[~self.df_scopus.DOI.isin(_idx_papers)][scopus_column].dropna().astype(int))
        _w_videos_cit = np.log10(self.df_scopus[self.df_scopus.DOI.isin(_idx_papers)][scopus_column].dropna().astype(int))
    ...
    def cluster_scopus(self, target_column="Abstract", num_clusters=None, train_kmeans=True, pca_components=2, pretrained_model="distilbert-base-nli-stsb-mean-tokens", videos_only=True):
        if type(pca_components) != int or pca_components > 3 or pca_components < 2:
            raise ValueError("[-]Argument 'pca_components' must be either 2 or 3.")
        ...
        if target_column == "Abstract":
            _df_scopus_filtered = self.df_scopus[self.df_scopus["Abstract"] != "[No abstract available]"]
        else:
        ...
        # Papers w/ videos
        _target_scopus = _df_scopus_filtered[_df_scopus_filtered["DOI"].isin(self.idx_papers)] if videos_only else _df_scopus_filtered
        ...
        for sentence_id, cluster_id in enumerate(self.clustering_model.labels_):  # Array of cluster index
            self.clustered_abs[cluster_id].append(_list_corpus[sentence_id])
            self.clustered_embeddings[cluster_id].append(self.corpus_embeddings[sentence_id])
        ...
        if pca_components == 2:
            ...
            ax.scatter(_corpus_pca[:, 0], _corpus_pca[:, 1], _corpus_pca[:, 2], c=self.clustering_model.labels_)
            plt.show()
        
        return self
    
    def desc_clusters(self, cluster_key):
        _set_titles = set(self.df_scopus[self.df_scopus["DOI"].isin(self.idx_papers)]["Title"])  # For subjects only
        ...
        for _i, _cluster in enumerate(self.dict_key_clusters[cluster_key]):
            ...
            if cluster_key == "subjects":
                # Intersection
                _intersection = set(self.clustered_titles[_i]).intersection(_set_titles)
    ...
    def __desc_journals_scores(self, partition=5):
        ...
        for _i in range(0, partition):
            _target = self.journals_papers_w_videos.iloc[int(_size * _i): int(_size * (_i + 1))]
            _list_sum_journals.append(len(_target[_target != 0]))
            _list_sum_papers.append(_target.sum())
        
        print("Journals:\tMean: %.1f\tStd: %.2f" % (np.mean(_list_sum_journals), np.std(_list_sum_journals)))
        print("Papers:\tMean: %.1f\tStd: %.2f" % (np.mean(_list_sum_papers), np.std(_list_sum_papers)))

        _norm_journals, _norm_papers = stats.zscore(_list_sum_journals), stats.zscore(_list_sum_papers)
        for _i in range(0, partition):
            print("Partition %d of %d" % (_i + 1, partition))
            print("# journals w/ videos(Z-value): %d(%.2f)\t# papers w/ videos(Z-value): %d(%.2f)" % (_list_sum_journals[_i], _norm_journals[_i], _list_sum_papers[_i], _norm_papers[_i]))
